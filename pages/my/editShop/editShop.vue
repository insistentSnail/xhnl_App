<template>
	<view class="articleView">
		<view class="list">
			...
			<input type="text" autocomplete="off" placeholder="商品名称" v-model="shopInfo.title"  class="uni-input fl" />
		</view>
		...
		<view class="list type">
			...
			<input type="number" autocomplete="off" placeholder="请输入价格" v-model="shopInfo.price"  class="uni-input fl" />
		</view>
		<view class="list">
			<view class="des">
				联系电话
			</view>
			<input type="number" disabled autocomplete="off" placeholder="请输入联系电话" v-model="shopInfo.phone"  class="uni-input fl" />
		</view>
		<view class="list">
		...
		<view class="detail">
			...
			<view class="detailContent">
				<textarea class="textarea" placeholder="请输入详细地址" @blur="bindTextAreaBlur" :value="shopInfo.address" auto-height />
			</view>
		</view>
		<!-- 图片上传 -->
...
<script>
	import {goodsInfo,editGoodsInfo} from '@/api/my/my.js'
	import pickRegions from '@/components/pick-regions/pick-regions.vue'
	import common from '@/api/common.js'
	import {getShopTabs,shopList,addGoods,shopType} from '@/api/shop/shop.js'
	export default {
		components: {
			pickRegions
		},
		...
		onLoad(e) {
			this.id = e.id;
			uni.removeStorageSync('editShopText');
			...
			.then(res=>{
				...
				this.text = this.shopInfo.info;
				// this.regionName = this.shopInfo.province;
				// 回显城市三级联动
				var arr = this.shopInfo.province.split(" ");
				var newArr=[];
				...
				this.src = this.shopInfo.image;
				this.src2 = this.shopInfo.images;
				this.index = this.arr.findIndex((ele)=>
				  ele.id == this.shopInfo.cid
				)
			})
		},
		onShow() {
			this.text=uni.getStorageSync('editShopText');
		},
		methods: {
			// 三级联动城市选择
			   // 获取选择的地区
			...
			submit(){
				var arr = []
				for(var i=0;i<this.src2.length;i++){
					arr.push(this.src2[i].replace('http://app.yinyo.net',''))
				}
				...
				if(!(/^1[3456789]\d{9}$/.test(this.shopInfo.phone))){ 
					uni.showToast({
						title:"请输入正确的号码格式",
						icon:"none"
					})
